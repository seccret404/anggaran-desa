...
<div class="relative inline-flex" x-data="{ open: false }">
    <button class="inline-flex justify-center items-center group" aria-haspopup="true" @click.prevent="open = !open"
        ...
        <img class="w-8 h-8 rounded-full"
            src="@auth{{ Auth::user()->profile_photo_url }} @else https://api.dicebear.com/7.x/identicon/svg?seed=UserName" @endauth width="32"
            height="32" alt="@auth{{ Auth::user()->name }} @endauth" />
        <div class="flex items-center truncate">
            <span
                class="truncate ml-2 text-sm font-medium text-gray-600 dark:text-gray-100 group-hover:text-gray-800 dark:group-hover:text-white">@auth{{ Auth::user()->name }}@endauth
            </span>
    ...
    <div class="origin-top-right z-10 absolute top-full min-w-44 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1 {{ $align === 'right' ? 'right-0' : 'left-0' }}"
        ...
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-gray-200 dark:border-gray-700/60">
            <div class="font-medium text-gray-800 dark:text-gray-100">@auth{{ Auth::user()->name }}@endauth
            </div>
